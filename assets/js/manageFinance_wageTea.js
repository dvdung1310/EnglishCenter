...
function hienthids(status) {
    ...
    if (filteredData.length != 0) {
        for (var i = 0; i < filteredData.length; i++) {
            ...
            else if (filteredData[i]['TrangThai'] === 'Chưa thanh toán') { color = "#ff9393" }
            // else { color = "#bcbdff" }
...
// Danh sách giáo viên
  ...
  function filterTeachers() {
    ...
    var inputText = document.getElementById("name-teacher-add-bill").value.toLowerCase();
    ...
    else{var filteredTeachers = dsgv.filter(function(teacher) {
      return teacher.TenGV.toLowerCase().includes(inputText);
    });
    ...
    filteredTeachers.forEach(function(teacher) {
      ...
      liElement.setAttribute('cursor', 'pointer');
...
document.getElementById('sumit-bill-add-ps').addEventListener('click', function (event) {
    ...
    const class_bil = document.getElementById('bill-teacher-add-ps').value;
